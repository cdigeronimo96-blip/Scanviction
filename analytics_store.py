...
def daily_active(hours=24):
    """COUNT(DISTINCT uid) with any event in the last `hours` — the real DAU figure
    for the admin overview. None if the DB isn't available (caller falls back)."""
    def _do(conn):
        with _DB_LOCK:
            _ensure_table_locked(conn)
            with conn.cursor() as cur:
                cur.execute("SELECT COUNT(DISTINCT uid) FROM msp_events "
                            "WHERE uid IS NOT NULL AND ts >= now() - make_interval(hours => %s)",
                            (int(hours),))
                row = cur.fetchone()
