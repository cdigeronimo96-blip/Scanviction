...
def render_market_timer():
    ...
    <div style="display:flex;align-items:center;justify-content:center;gap:14px;
      ...
      <span style="font-size:13px;color:#6b7fa0;">{verb_txt}
        <span style="font-family:'JetBrains Mono',monospace;font-weight:800;color:#93b4fd;margin-left:4px;">{cd}</span>
      </span>
    </div>
    """, unsafe_allow_html=True)
...
/* ── Navigation transition (anti-flicker, consistent fade) ─────────────────
   On every rerun Streamlit briefly keeps the PREVIOUS page's elements in the
   DOM (marked data-stale) until the new frame paints. Putting a TRANSITION on
   those (fade-out) is what caused the "old boxes flash on and off" bug, because
   the fade-out fought the new frame's fade-in. Fix:
     1. Hide stale page content INSTANTLY (no transition) — the old page never
        visibly redraws, so there is no flicker.
     2. Fade the NEW page BODY in once, smoothly and consistently.
     3. EXEMPT the header (logo + nav buttons + username): it never fades and
        its stale copy stays visible, so the whole header stays rock-steady
        across navigations. */

/* 1. Instant-hide stale content (kills the on/off flicker). */
[data-stale="true"]{{ display:none !important; }}

/* 3. Header stays put — identified by the nav buttons' .sw-tb-btn class.
   Keep it visible even when marked stale, and never animate it. */
div[data-testid="stHorizontalBlock"]:has(.sw-tb-btn),
div[data-testid="stHorizontalBlock"]:has(.sw-tb-btn)[data-stale="true"]{{
    display:flex !important; opacity:1 !important; animation:none !important;
}}
div[data-testid="stHorizontalBlock"]:has(.sw-tb-btn) *{{ animation:none !important; }}

/* 2. Smooth, consistent fade-in for page BODY content only. We animate the
   non-header children of the main vertical block so the header is untouched. */
[data-testid="stMainBlockContainer"] > div[data-testid="stVerticalBlock"] > div:not(:has(.sw-tb-btn)){{
    animation:mspFadeIn 0.4s ease both;
}}
        ...
        if tok:
            ...
            if cm is not None:
                try:
                    ...
                    pass
            # NOTE: we deliberately do NOT mirror the sid to localStorage.
            # It was write-only (never read — session restores from the URL
            # ?sid= and the cookie), and the components.html() iframe injection
            # it required triggered a rerun on first sign-in, which produced the
            # visible double-render ("Good evening" + Market Overview twice).
            # The cookie + URL already persist the session durably.
        return
...
UNIVERSE_REFRESH        = 3600   # worker rebuild: every 60 min during market hours
...
_UNIVERSE_CACHE = {"rows": [], "built_at": 0.0, "hot": [], "attempted": False,
...
def _universe_worker():
    ...
    while True:
        ...
        try:
            ...
            with _UNIVERSE_LOCK:
                _scored = _UNIVERSE_CACHE.get("ok", 0)
        ...
        # Critical for the free TD tier: don't burn credits when the market
        # is closed (prices aren't moving). On weekends, sleep ~indefinitely
        # (24h) — quotes don't change and we'd waste the whole next-day
        # budget before users wake up.
        try:
            _ms = market_status()
            _state = _ms.get("state", "open")
        except Exception:
            _state = "open"
        if _state == "closed":  # weekend
            _sleep_for = UNIVERSE_REFRESH_WEEKEND or 86400
        elif _state in ("pre", "after"):
            _sleep_for = UNIVERSE_REFRESH_CLOSED
        else:
            _sleep_for = UNIVERSE_REFRESH
...
def render_sr(s, cat_key="", show_why=False, cat_name="", snap=None):
    ...
    with col_btn:
        ...
        if st.button("✅ Saved" if in_wl else "➕ Watch",key=f"wl_{t}_{cat_key}",use_container_width=True,type="primary" if not in_wl else "secondary"):
            _toggle_watchlist(t)
            st.rerun()
...
def _render_warming_fragment(cat):
    """Isolated auto-refreshing box shown while the first universe warm runs.
    Because it's a fragment with run_every, ONLY this box re-executes every 2s —
    the rest of the page (topbar, header, category grid) stays put, eliminating
    the full-page repaint flicker/duplication. When the warm finishes, we do one
    full-app rerun to render the actual results."""
    in_progress = False
    try: in_progress = _WARM_IN_PROGRESS
    except Exception: pass
    if not (universe_is_warming() or in_progress):
        # Data is ready (or the attempt finished) — rerun the whole app once so
        # the normal results path renders.
        st.rerun(scope="app")
        return
    st.markdown('''<div style="background:#0d1525;border:1px solid rgba(37,99,235,0.25);
                       border-radius:12px;padding:30px;text-align:center;">
        <div style="display:flex;align-items:center;justify-content:center;gap:12px;">
          <div style="width:18px;height:18px;border:2px solid rgba(37,99,235,0.25);border-top-color:#60a5fa;border-radius:50%;animation:spin 0.8s linear infinite;"></div>
          <div style="text-align:left;">
            <div style="font-size:15px;font-weight:700;color:#e2e8f0;">Preparing live market data…</div>
            <div style="font-size:12px;color:#374f6e;">Scoring the universe — categories switch instantly after this.</div>
          </div>
...
def render_cat(cat,limit=10,show_why=False):
    ...
    if not stocks:
        ...
        if universe_is_warming() or _in_progress:
            # Poll inside an ISOLATED FRAGMENT so only this little box refreshes —
            # NOT the whole page. The old approach (time.sleep + st.rerun) re-ran
            # the entire script every 1.3s, which repainted the topbar/greeting/
            # everything and caused the visible "duplicate boxes that pop into
            # place" flicker. A fragment with run_every reruns just itself.
            _render_warming_fragment(cat)
            return
...
.element-container:has(.msp-logo-click-target)+.element-container .stButton>button{position:relative !important;top:-44px !important;left:0 !important;width:150px !important;height:44px !important;min-height:44px !important;opacity:0 !important;cursor:pointer !important;z-index:999 !important;background:transparent !important;border:none !important;box-shadow:none !important;}
...
def render_topbar(active=""):
    ...
    .sw-tb-btn .stButton>button {{
        ...
        color:#8a9ab5 !important; font-size:12px !important;
        font-weight:500 !important; height:34px !important;
        min-height:34px !important; padding:0 4px !important;
    ...
    /* ── User-menu trigger pill ── */
    .sw-tb-usermenu [data-testid="stPopover"] > div > button,
    .sw-tb-usermenu button[data-testid="stPopoverButton"] {{
        background:rgba(255,255,255,0.05) !important;
        border:1px solid rgba(255,255,255,0.14) !important;
        color:#dbe6f5 !important; font-size:13px !important;
    ...
    .sw-tb-usermenu [data-testid="stPopover"] > div > button:hover {{
        border-color:rgba(37,99,235,0.5) !important;
        background:rgba(37,99,235,0.12) !important; color:#fff !important;
    }}
    /* Style Streamlit's built-in chevron so it looks intentional (not a glyph
       stacked on a glyph). Keep just the one clean chevron. */
    .sw-tb-usermenu [data-testid="stPopover"] svg {{
        opacity:0.6 !important; margin-left:4px !important;
    }}
    /* ── Dropdown panel ── */
    /* The BaseWeb popover wraps our body in its own div with a white bg — make
       the whole popover surface dark so there's no white frame/clipping. */
    ...
    [data-testid="stPopoverBody"] {{
        ...
        border-radius:14px !important;
        box-shadow:0 18px 50px rgba(0,0,0,0.6) !important;
        padding:10px !important; min-width:248px !important;
    }}
    /* Header with avatar */
    .um-head {{ display:flex; align-items:center; gap:11px; padding:6px 6px 12px; }}
    .um-avatar {{ width:40px; height:40px; border-radius:50%; flex:0 0 40px;
        background:linear-gradient(135deg,{BLUE_LOC},#7c3aed);
        display:flex; align-items:center; justify-content:center;
        font-size:17px; font-weight:800; color:#fff; }}
    .um-id {{ min-width:0; }}
    .um-name {{ font-size:14px; font-weight:700; color:#f1f5f9; line-height:1.2;
        white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }}
    .um-email {{ font-size:11.5px; color:#7e90ac; white-space:nowrap;
        overflow:hidden; text-overflow:ellipsis; }}
    .um-sep {{ height:1px; background:rgba(255,255,255,0.08); margin:4px 2px; }}
    /* Dropdown list buttons — flat, list-style, left-aligned, no boxy borders */
    [data-testid="stPopoverBody"] .stButton>button {{
        background:transparent !important;
        border:1px solid transparent !important;
        color:#c7d4e6 !important; justify-content:flex-start !important;
    ...
    if is_authed():
        pages = [("Home","dashboard"),("Discover","discover"),("Watch","watchlist"),
                 ("Screen","screener"),("BI","bi_dashboard"),("Pricing","pricing"),("Contact","contact")]
        ...
        # Columns sized to roughly match each label's width so buttons look
        # even with consistent gaps (equal ratios made "BI" and "Discover" look
        # mismatched). Logo first, user-menu last.
        _label_w = {"Home":0.8,"Discover":1.15,"Watch":0.85,"Screen":0.9,
                    "BI":0.6,"Pricing":0.9,"Contact":1.0,"Admin":0.85}
        nav_ratios = [_label_w.get(lbl, 1.0) for lbl, _ in pages]
        ratios = [2.1] + nav_ratios + [1.5]
        ...
        # User menu — click the name to open a clean dropdown (avatar header +
        # Watchlist, Alerts, Settings, Log out). No manual arrow glyph (Streamlit
        # renders its own chevron, which we style); this avoids the double-arrow.
        with cols[len(pages) + 1]:
            st.markdown('<div class="sw-tb-usermenu">', unsafe_allow_html=True)
            ...
            with st.popover(f"{first}", use_container_width=True):
                st.markdown(
                    ...
                    f'<div class="um-id"><div class="um-name">{first}</div>'
                    f'<div class="um-email">{_email}</div></div>'
                    f'</div><div class="um-sep"></div>', unsafe_allow_html=True)
...
def page_discover():
    ...
    .disc-section-label{{font-size:11px;font-weight:800;color:#4a5e7a;letter-spacing:2.5px;
        text-transform:uppercase;margin:22px 0 12px;text-align:center;}}
    ...
    # ── Live market countdown timer (centered, ticks in-browser) ──
    render_market_timer()
    ...
    _dh += "<span class='disc-meta-pill'>📊 Yahoo Finance data</span>"
    _dh += "<span class='disc-meta-pill'>🔄 Updates every session</span>"
    _dh += "</div></div>"
    st.markdown(_dh, unsafe_allow_html=True)

    # ── Background warm runs SILENTLY ──
    # The worker pre-scores the universe in the background (kicked on page load
    # via ensure_universe_worker above). We intentionally show NO status bar /
    # spinner / "Analyze All" button to regular users — categories simply become
    # ready on their own, and individual categories show their own lightweight
    # "preparing…" state only if opened before the first warm finishes. Admins
    # can force a re-scan from Settings → System → "Refresh data now".
    #
    # If the warm genuinely FAILED (attempted, no rows, data provider down), we
    # surface a single quiet inline notice so the page isn't mysteriously empty.
    with _UNIVERSE_LOCK:
        _warm_n = len(_UNIVERSE_CACHE["rows"])
        _attempted = _UNIVERSE_CACHE.get("attempted", False)
    _kick_running = False
    try: _kick_running = _WARM_IN_PROGRESS
    except Exception: pass
    _failed = (_warm_n == 0) and _attempted and not _kick_running
    if _failed:
        st.markdown(f'<div style="display:flex;align-items:center;gap:8px;background:#0d1525;'
                    f'border:1px solid rgba(251,191,36,0.25);border-radius:10px;padding:10px 14px;'
                    f'font-size:12px;color:{GOLD};margin-bottom:8px;">'
                    f'⚠️ Live market data is temporarily unavailable. Showing the latest available; '
                    f'it will refresh automatically.</div>', unsafe_allow_html=True)
    ...
    for row_start in range(0, len(comp_items), 4):
        ...
        chunk = comp_items[row_start:row_start+4]
        for col_idx in range(4):
            with cols[col_idx]:
                if col_idx >= len(chunk):
                    continue  # keep the grid rectangular so rows stay aligned
                cat, (desc, tier) = chunk[col_idx]
    ...
    for row_start in range(0, len(std_items), 4):
        cols = st.columns(4, gap="small")
        chunk = std_items[row_start:row_start+4]
        for col_idx in range(4):
            with cols[col_idx]:
                if col_idx >= len(chunk):
                    continue
    ...
    st.markdown('<div style="height:8px;"></div><div style="border-top:1px solid rgba(255,255,255,0.06);margin:8px 0 18px;"></div>', unsafe_allow_html=True)

    # ── Results area ──
    # render_cat owns ALL three loading states (warming / failed / ready) so we
    # do NOT wrap it in an outer "Preparing…" placeholder — doing so showed the
    # message twice (the outer box plus render_cat's own), and the outer
    # placeholder.empty() never ran because render_cat reruns during warming.
    if is_locked:
        render_lock(sel)
    else:
