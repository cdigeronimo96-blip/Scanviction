...
/* Hide Streamlit's "stale" element overlay. During a rerun, Streamlit keeps
   the previous frame's elements in the DOM (marked data-stale / .stale) and
   dims them until the new frame is ready. On pages with a brief data-load this
   showed as DUPLICATED content (e.g. the dashboard greeting + Market Overview
   appearing twice). Hiding stale elements means the user sees only the current
   frame — no ghost copies. */
...
def render_topbar(active=""):
    ...
    /* User-menu popover trigger — looks like a nav pill, opens dropdown */
    .sw-tb-usermenu [data-testid="stPopover"] > div > button,
    .sw-tb-usermenu button[data-testid="stPopoverButton"] {{
        background:rgba(255,255,255,0.04) !important;
        border:1px solid rgba(255,255,255,0.14) !important;
        color:#c7d4e6 !important; font-size:12.5px !important;
    ...
    /* Dark dropdown panel to match the theme (Streamlit's default is light) */
    ...
    [data-testid="stPopoverBody"] .stButton>button:hover {{
        background:rgba(37,99,235,0.12) !important;
        border-color:rgba(37,99,235,0.4) !important; color:#93b4fd !important;
    }}
    ...
    if is_authed():
        ...
        # User menu — click the name to open a dropdown with Settings + Logout
        # (replaces the separate non-clickable label + gear + logout icons).
        with cols[len(pages) + 1]:
            st.markdown('<div class="sw-tb-usermenu">', unsafe_allow_html=True)
            with st.popover(f"{ri} {first}  ⌄", use_container_width=True):
                st.markdown(f'<div style="font-size:12px;color:#6b7fa0;padding:2px 2px 8px;">'
                            f'Signed in as<br><b style="color:#e2e8f0;">{st.session_state.user.get("email","")}</b></div>',
                            unsafe_allow_html=True)
                if st.button("⚙️  Account Settings", key="um_settings", use_container_width=True):
                    nav("settings")
                if st.button("↩️  Log out", key="um_logout", use_container_width=True):
                    logout()
            st.markdown('</div>', unsafe_allow_html=True)
