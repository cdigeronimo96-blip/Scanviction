...
/* CRITICAL: the decorative .page-wrap div above does NOT constrain the actual
   Streamlit widgets (columns/buttons), which render in Streamlit's own
   full-width block container. To make styled content and widgets line up — and
   to stop category buttons overflowing the right edge — we constrain and center
   Streamlit's real content container here. */
...
@media(max-width:900px){{
    ...
    [data-testid="stMainBlockContainer"], .block-container{{padding-left:16px !important;padding-right:16px !important;}}
}}
...
def _td_usage_check_and_increment(n=1):
    ...
    Resets at UTC midnight (matches Twelve Data's reset behavior)."""
    from datetime import datetime as _dtu
    today = _dtu.utcnow().strftime("%Y-%m-%d")
    with _TD_USAGE_LOCK:
        if _TD_USAGE["date"] != today:
            _TD_USAGE["date"] = today; _TD_USAGE["credits"] = 0
...
def td_usage_today():
    ...
    today = _dtu.utcnow().strftime("%Y-%m-%d")
    with _TD_USAGE_LOCK:
        return {"date": _TD_USAGE["date"], "used": _TD_USAGE["credits"],
                "budget": TWELVE_DATA_DAILY_BUDGET,
                "remaining": max(0, TWELVE_DATA_DAILY_BUDGET - _TD_USAGE["credits"]) if _TD_USAGE["date"] == today else TWELVE_DATA_DAILY_BUDGET}
...
FMP_MAX_POOL = 120               # cap how many extra tickers we pre-score (yfinance rate-limits hard above ~150 rapid calls; keep warming fast & reliable)
...
def render_cat(cat,limit=10,show_why=False):
    ...
    if not stocks:
        ...
        if universe_is_warming() or _in_progress:
            # Keep polling while the worker is genuinely still building. We cap
            # the poll count only to avoid an unbounded rerun storm, but if the
            # worker is still in progress we keep waiting (it WILL terminate the
            # state when done — universe_is_warming flips False after attempt).
            ...
            if attempts < 15:
                ...
                st.rerun()
                return
            # Polled long enough and still nothing — show the retry state.
            st.session_state["_warm_attempts"] = 0
            st.markdown('''<div style="background:#0d1525;border:1px solid rgba(251,191,36,0.3);
                               border-radius:12px;padding:30px;text-align:center;">
                <div style="font-size:24px;margin-bottom:10px;">⏳</div>
                <div style="font-size:15px;font-weight:700;color:#e2e8f0;margin-bottom:6px;">Still preparing data…</div>
                <div style="font-size:13px;color:#6b7fa0;max-width:460px;margin:0 auto;">The first scan is taking a while (the data provider may be rate-limiting). It will keep trying in the background.</div>
            </div>''', unsafe_allow_html=True)
            if st.button("🔄 Check again", key=f"warm_poll_{cat[:8]}", use_container_width=True):
                st.rerun()
            return
        st.session_state["_warm_attempts"] = 0
...
def page_discover():
    ...
    /* Equal-height, neatly-aligned category buttons */
    .stButton>button{{min-height:46px;border-radius:10px;font-weight:600;
    ...
    # State: warming (in progress / not yet attempted) | failed | ready
    _still_warming = (_warm_n == 0) and (_kick_running or not _attempted)
    _failed = (_warm_n == 0) and _attempted and not _kick_running
    _sb1, _sb2 = st.columns([3,1], gap="medium")
    with _sb1:
        if _still_warming:
            st.markdown(f'<div style="display:flex;align-items:center;gap:10px;background:#0d1525;'
                        f'border:1px solid rgba(37,99,235,0.25);border-radius:10px;padding:11px 16px;font-size:12.5px;color:#93b4fd;min-height:44px;box-sizing:border-box;">'
                        f'<span class="loader-spin" style="width:14px;height:14px;border:2px solid rgba(37,99,235,0.25);border-top-color:#60a5fa;border-radius:50%;display:inline-block;animation:spin 0.8s linear infinite;"></span>'
                        f'Analyzing all categories… runs once, then every category is instant.</div>'
                        f'<style>@keyframes spin{{to{{transform:rotate(360deg);}}}}</style>', unsafe_allow_html=True)
        elif _failed:
            st.markdown(f'<div style="display:flex;align-items:center;gap:8px;background:#0d1525;'
                        f'border:1px solid rgba(251,191,36,0.3);border-radius:10px;padding:11px 16px;font-size:12.5px;color:{GOLD};min-height:44px;box-sizing:border-box;">'
                        f'⚠️ Couldn\'t load market data (provider may be rate-limiting). Click Analyze All to retry.</div>', unsafe_allow_html=True)
        else:
            _scope_badge = ("🌐 Market-wide" if _market_wide else "📋 Curated universe")
            st.markdown(f'<div style="display:flex;align-items:center;gap:8px;background:#0d1525;'
                        f'border:1px solid rgba(34,197,94,0.2);border-radius:10px;padding:11px 16px;font-size:12.5px;color:#4ade80;min-height:44px;box-sizing:border-box;">'
                        f'✅ Ready · {_scope_badge} <span style="color:#374f6e;">· {_warm_n} of {_scanned} scored · {_age}</span></div>', unsafe_allow_html=True)
    with _sb2:
        if st.button("🔄 Analyze All", key="disc_analyze_all", use_container_width=True,
            ...
            st.session_state["_warm_attempts"] = 0
            st.toast("Re-analyzing all categories…", icon="⚡")
            st.rerun()
