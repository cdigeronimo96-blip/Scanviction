...
/* Loading polish: when Streamlit reruns, it keeps the previous frame's elements
   (marked data-stale) dimmed until the new frame is ready. Hard-hiding them made
   content "pop in" piecemeal. Instead we fade stale content to near-zero opacity
   quickly (so duplicates aren't visible) WITHOUT removing layout — the new frame
   fades in over it smoothly, so a rerun reads as one clean transition rather than
   a flash of stacked/duplicated boxes. */
[data-stale="true"]{{
    opacity:0 !important;
...
[data-testid="stMainBlockContainer"]{{
    animation:mspContentIn 0.22s ease-out;
}}
@keyframes mspContentIn {{ from{{opacity:0.6;}} to{{opacity:1;}} }}
