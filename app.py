        ...
        if tok:
            ...
            if cm is not None:
                try:
                    ...
                    pass
            # Mirror to localStorage ONCE per session, not on every render.
            # Each components.html() call creates an iframe Streamlit has to
            # mount; doing it every page load caused visible layout shifts and
            # made navigation feel "buggy"/duplicated.
            if not st.session_state.get("_sid_ls_mirrored"):
                components.html(
                    f"<script>try{{localStorage.setItem('msp_sid','{tok}');}}catch(e){{}}</script>",
                    height=0,
                )
                st.session_state["_sid_ls_mirrored"] = True
        return
...
def render_topbar(active=""):
    ...
    if is_authed():
        ...
        # Columns: logo | ...nav items... | user-menu (popover)
        ratios = [2.3] + [1.05]*len(pages) + [1.4]
