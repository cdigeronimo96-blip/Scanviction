...
def render_market_timer():
    """A clean, centered live market-status banner with a JS countdown that
    ticks every second in the browser (no Streamlit reruns)."""
    ms = market_status()
    ...
    components.html(f"""
    <div id="mkt-timer" style="display:flex;align-items:center;justify-content:center;gap:14px;
        font-family:'Inter',-apple-system,sans-serif;padding:11px 18px;margin:0 auto 6px;
      ...
      <span style="font-size:13px;color:#6b7fa0;">{verb_txt}
        <span id="mkt-cd" style="font-family:'JetBrains Mono',monospace;font-weight:800;color:#93b4fd;margin-left:4px;">--:--:--</span>
      </span>
    </div>
    <script>
      (function(){{
        ...
        function tick(){{ if(el) el.textContent = fmt(s); s--; if(s<-2){{clearInterval(iv);}} }}
