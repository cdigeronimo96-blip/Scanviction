...
def _kick_background_warm(force=False):
    ...
    if force:
        # let the warming UI re-enter its wait state for a fresh attempt
        with _UNIVERSE_LOCK:
...
def render_cat(cat,limit=10,show_why=False):
    ...
    if not stocks:
        # Three distinct states now, no infinite loop possible:
        #  (1) still warming  → first attempt hasn't finished; poll briefly
        ...
        if universe_is_warming():
            ...
            if attempts < 8:
                ...
                st.rerun()
                return
            st.session_state["_warm_attempts"] = 0  # fall through to retry UI below
        ...
        with tabs[6]:
            ...
            if _is_db:
                ...
                st.markdown(f'<div style="background:#1a1400;border:1px solid rgba(245,158,11,0.3);border-radius:10px;'
                            f'padding:12px 16px;margin-bottom:14px;font-size:12px;color:{GOLD};">⚠️ <b>File storage.</b> '
                            f'Data is saved to <code>{USERS_DB_PATH}</code>. This survives a normal restart only if your host preserves that path. '
                            f'For guaranteed persistence across redeploys, set <code>DATABASE_URL</code> (Postgres).</div>', unsafe_allow_html=True)
