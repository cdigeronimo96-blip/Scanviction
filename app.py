...
/* ── Page transition ───────────────────────────────────────────────────────
   Streamlit reruns the whole script on every interaction; by default it keeps
   the previous frame's elements visible (dimmed) until the new frame paints.
   We DON'T hide those stale elements — hiding them blanked the page and made
   content "fall in from the top." Instead we leave Streamlit's default
   in-place update and apply only a gentle opacity fade on the body content
   (no transform, so nothing slides/falls). The header is exempted entirely so
   it updates in place without fading. */
[data-stale="true"]{{ opacity:0.999; }}  /* no-op: never hide/animate stale */
@keyframes mspFadeIn {{ from{{opacity:0.35;}} to{{opacity:1;}} }}
        ...
        for col_idx in range(4):
            with cols[col_idx]:
                ...
                if st.button(lbl, key=f"disc_c_{safe}_{row_start}_{col_idx}", use_container_width=True, type=btn_type):
                    if is_l: nav("pricing")
                    else: st.session_state.discover_cat=cat; st.rerun(scope="fragment")
        ...
        for col_idx in range(4):
            with cols[col_idx]:
                ...
                if st.button(cat, key=f"disc_s_{cat[:24].replace(' ','_')}_{row_start}_{col_idx}", use_container_width=True, type=btn_type):
                    st.session_state.discover_cat=cat; st.rerun(scope="fragment")
