...
PAGE_ACCESS = {
    ...
    "stock_detail": "auth", "settings": "auth", "signal_track": "auth",
    # premium
...
def _build_universe_raw():
    """Score the full universe via the tiered cache (no Streamlit dependency)."""
    hot = _tiered_get("__hot__", "hot", MOD_TTL, lambda _: _raw_hot())
    universe = list(dict.fromkeys(BROAD_UNI + (hot or [])[:8]))[:32]
    rows = []
    for t in universe:
...
def build_scored_universe():
    ...
    Fast path: a background thread keeps _UNIVERSE_CACHE fresh, so this returns
    instantly with near-live data. Cold path (very first call before the worker
    has produced anything): build once synchronously, seed the cache, and make
    sure the worker is running for subsequent refreshes.
    """
    ensure_universe_worker()
    with _UNIVERSE_LOCK:
        rows = list(_UNIVERSE_CACHE["rows"])
        built_at = _UNIVERSE_CACHE["built_at"]
    if rows:
        return rows
    # Cold start — nothing warm yet. Build once inline so this request isn't blank.
    return _refresh_universe_now()
...
def render_sr(s, cat_key="", show_why=False, cat_name=""):
    ...
    # ── Performance since signal (from the snapshot store) ──
    perf_html = ""
    snap = get_recommendation_snapshot(cat_name, t) if cat_name else None
    if snap:
...
def render_cat(cat,limit=10,show_why=False):
    ...
    if is_comp:
        ...
        # Use the cached per-ticker scorer so repeat visits are instant.
        with st.spinner(f"Analyzing {cat}…"):
            for t in tickers[:scan]:
                r=score_ticker(t)
                if r:
                    r=dict(r); r["hot"]=t in hot; r["comp"]=r["sc"]; r["why"]=""
                    stocks.append(r)
    ...
    if not stocks:
        ...
        return
    # ── Persist a recommendation snapshot for each surfaced signal ──
    # First sighting captures entry price + timestamp; later refreshes update
    # the live price. This powers "performance since signal" and the $1000 math.
    for s in stocks:
        try:
            record_recommendation(
                category=cat, ticker=s.get("t",""),
                price=(s.get("q") or {}).get("price", 0),
                score=s.get("sc"), recommendation=s.get("op"), why=s.get("why"),
            )
        except Exception:
            pass
    for s in stocks: render_sr(s,cat.replace(" ","_").replace("+","p").replace("→","r"),show_why=is_comp,cat_name=cat)
