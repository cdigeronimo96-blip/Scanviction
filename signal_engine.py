...
def get_category_performance_stats() -> dict:
    ...
    for ev in events:
        if ev.get("demo"):
            continue   # seeded examples never count toward the real track record
...
def seed_demo_signal_history(price_fn=None):
    ...
    # Tag every seeded event as demo data so it can be excluded from real
    # performance stats and purged cleanly (see purge_demo_signal_history).
    for e in demo_events:
        e["demo"] = True
...
# Id prefixes used by historical seed runs BEFORE events carried the "demo": True tag.
# Real event ids come from _next_evt_id, which embeds the category emoji (e.g.
# "GME_🔥💥_Squeeze_+_Buzz_…"), so these plain-ASCII prefixes can't collide with them.
_LEGACY_DEMO_ID_PREFIXES = (
    "GME_Squeeze_Buzz_", "NVDA_Triple_Lock_", "PLTR_Hidden_Movers_",
    "AMD_Smart_Reversal_", "MSTR_Volume_Breakout_",
    "FLUT_insider_", "ON_8k_", "KYMR_si_",
)

def purge_demo_signal_history() -> int:
    """Remove ALL seeded demo events (tagged demo:True, plus legacy untagged seeds
    matched by id prefix) so category performance reflects ONLY real tracked signals.
    ...
    def _is_demo(e):
        if e.get("demo"):
            return True
        eid = str(e.get("id", ""))
        return any(eid.startswith(p) for p in _LEGACY_DEMO_ID_PREFIXES)
    kept = [e for e in events if not _is_demo(e)]
    removed = len(events) - len(kept)
    if removed:
        _write_json(SIGNAL_HISTORY_PATH, kept)
    return removed
